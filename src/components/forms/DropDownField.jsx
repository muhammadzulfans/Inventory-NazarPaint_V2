...
const DropDownField = ({ label, icon: Icon, options, value, onChange, placeholder }) => {
    ...
    return (
        <div className="font-inter relative text-left">
            ...
            {isOpen && (
                <ul className="absolute z-50 w-full mt-2 bg-white shadow-xl rounded-lg overflow-hidden border border-line animate-in fade-in slide-in-from-top-2 duration-200">
                    {options.map((opt) => (
                        <li
                            key={opt.value}
                            onClick={() => {
                            ...
                            // DISINI KITA BISA CUSTOM HOVER-NYA SEPUAS HATI
            ...
            {/* Backdrop untuk nutup dropdown kalau klik diluar */}
